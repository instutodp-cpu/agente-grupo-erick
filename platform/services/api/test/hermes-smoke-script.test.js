...
test('hermes smoke test json helpers pass JSON outside the Node stdin script stream', () => {
  const scriptPath = path.resolve(__dirname, '../../../scripts/hermes-smoke-test.sh');
  const script = fs.readFileSync(scriptPath, 'utf8');

  assert.match(script, /JSON_INPUT="\$json"\s+"\$NODE_BIN" - "\$path"/);
  assert.match(script, /JSON_INPUT="\$json"\s+"\$NODE_BIN" - "\$field"/);
  assert.match(script, /JSON_INPUT="\$json"\s+"\$NODE_BIN" - "\$fields"/);
  assert.equal(/printf '%s' "\$json" \| "\$NODE_BIN" -/.test(script), false);
  assert.match(script, /JSON\.parse\(process\.env\.JSON_INPUT\)/);
});
...
test('hermes smoke test normalizes JSON booleans and logs safe request diagnostics', () => {
  ...
  assert.match(script, /value \? "true" : "false"/);
