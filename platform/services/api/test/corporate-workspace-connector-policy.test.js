...
const docPath = path.resolve(__dirname, '../../../docs/CORPORATE_WORKSPACE_CONNECTOR_POLICY.md');
const fixturePath = path.resolve(
  __dirname,
  'fixtures/hermes-corporate-workspace-connector-policy.json',
);
...
const REQUIRED_BLOCKING_RULES = [
  'real_corporate_connector_before_readiness_gate',
  'oauth_real_without_policy',
  'token_secret_without_management',
  'personal_token_used_for_corporate',
  'external_client_token_used_for_corporate',
  'real_send_attempted',
  'real_publish_attempted',
  'real_create_update_delete_attempted',
  'base44_write_attempted',
  'supabase_write_attempted',
  'postgres_write_attempted',
  'raw_sql_attempted',
  'erp_writeback_attempted',
  'linx_writeback_attempted',
  'payment_purchase_stock_or_credit_attempted',
  'github_write_attempted',
  'codex_write_attempted',
  'claude_code_write_attempted',
  'corporate_connector_used_in_personal',
  'corporate_connector_used_in_external_client',
  'personal_context_used',
  'external_client_context_used',
  ...
  'prompt_organization_override_attempt',
  'provider_tenant_override_attempt',
  'store_scope_bypass_attempted',
  'department_scope_bypass_attempted',
  'permission_matrix_bypass_attempted',
...
test('corporate workspace connector policy document and fixture exist', () => {
  assert.equal(fs.existsSync(docPath), true);
  assert.equal(fs.existsSync(fixturePath), true);
});

test('corporate workspace connector policy fixture is safe and complete', () => {
  ...
  for (const candidate of REQUIRED_CONNECTOR_CANDIDATES) {
    assert.ok(contract.corporate_connector_candidates.includes(candidate), candidate);
  }
  ...
  for (const status of REQUIRED_STATUSES) {
    assert.ok(contract.corporate_connector_statuses.includes(status), status);
  }
  ...
  assert.equal(contract.default_rules.workspace_type_must_be_corporate, true);
  assert.equal(contract.default_rules.tenant_id_required, true);
  assert.equal(contract.default_rules.tenant_id_must_be_grupo_erick, true);
  assert.equal(contract.default_rules.organization_id_required, true);
  assert.equal(contract.default_rules.organization_id_must_be_grupo_erick, true);
  assert.equal(contract.default_rules.user_id_required, true);
  assert.equal(contract.default_rules.role_required, true);
  ...
  assert.equal(contract.default_rules.external_client_context_allowed, false);
  ...
  for (const example of contract.safe_corporate_connector_examples) {
