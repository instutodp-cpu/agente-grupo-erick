...
const { validateWorkflowDecision } = require('./workflow-decision');

const WORKFLOW_REGISTRY_VALIDATOR_VERSION = 'workflow_registry_validator_v1';
const WORKFLOW_REGISTRY_STATUSES = Object.freeze([
...
const WORKFLOW_REGISTRY_SAFE_FLAGS = Object.freeze({
  simulation: true,
...
function safe(payload) {
  return cloneFrozen({ ...payload, ...WORKFLOW_REGISTRY_SAFE_FLAGS });
}
...
function createWorkflowRegistry() {
  const workflowStore = createEntityStore({
    idField: 'workflow_id', tenantField: 'tenant_id', organizationField: 'organization_id',
    versionField: 'workflow_version', validate: validateWorkflowContract, idLabel: 'workflow'
  });
  const stepStore = createEntityStore({
    idField: 'step_id', versionField: 'step_version', validate: validateWorkflowStep, idLabel: 'step'
  });
  const decisionStore = createEntityStore({
    idField: 'decision_id', tenantField: 'tenant_id', organizationField: 'organization_id',
    validate: validateWorkflowDecision, idLabel: 'decision'
  });

  return Object.freeze({
    registerWorkflow: workflowStore.register,
    getWorkflowById: workflowStore.getById,
    listWorkflowsByTenant: workflowStore.listByTenant,
    listWorkflowsByOrganization: workflowStore.listByOrganization,

    registerStep: stepStore.register,
    getStepById: stepStore.getById,
...
module.exports = {
  MAX_LIST_RESULTS,
  WORKFLOW_REGISTRY_SAFE_FLAGS,
  WORKFLOW_REGISTRY_STATUSES,
  WORKFLOW_REGISTRY_VALIDATOR_VERSION,
