...
const RUNTIME_QUEUE_MATERIALIZATION_AUDIT_VALIDATOR_VERSION = 'runtime_queue_materialization_audit_validator_v1';
...
// class reference IDs, counts, and blockers/reason codes -- never a payload, prompt, memory,
// message, tool argument, secret, endpoint, provider output, sensitive token, or credential. Mirrors
// runtime-queue-admission-audit.js's own shape exactly, one layer up.
const RUNTIME_QUEUE_MATERIALIZATION_AUDIT_FIELDS = Object.freeze([
  'audit_id', 'runtime_queue_materialization_request_id', 'runtime_queue_materialization_decision_id',
  'runtime_queue_materialization_result_id', 'runtime_queue_materialization_package_id',
  ...
  'queue_class_reference_ids', 'entry_counts',
...
const FINGERPRINT_KEYS = Object.freeze([
  ...
  'runtime_queue_materialization_package_fingerprint', 'runtime_queue_admission_package_fingerprint'
]);
const ENTRY_COUNT_KEYS = Object.freeze(['entry_count', 'materialized_count', 'not_materialized_count']);
...
function validateRuntimeQueueMaterializationAudit(audit) {
  const errors = [];
  if (!isPlainObject(audit)) return { valid: false, errors: ['runtime_queue_materialization_audit_must_be_object'] };
  exactFields(audit, RUNTIME_QUEUE_MATERIALIZATION_AUDIT_FIELDS, 'runtime_queue_materialization_audit', errors);
  for (const field of [
    'audit_id', 'runtime_queue_materialization_request_id', 'runtime_queue_materialization_decision_id',
    'runtime_queue_materialization_result_id', 'runtime_queue_materialization_package_id',
    'runtime_queue_admission_package_id', 'package_digest',
    'status', 'decision', 'next_state', 'validator_version'
  ]) {
    if (!isNonEmptyString(audit[field])) errors.push(`${field}_invalid`);
  }
  if (!QUEUE_MATERIALIZATION_STATUSES.includes(audit.status)) errors.push('status_invalid');
  if (!QUEUE_MATERIALIZATION_DECISIONS.includes(audit.decision)) errors.push('decision_invalid');
  if (!QUEUE_MATERIALIZATION_NEXT_STATES.includes(audit.next_state)) errors.push('next_state_invalid');
  ...
  if (audit.validator_version !== RUNTIME_QUEUE_MATERIALIZATION_AUDIT_VALIDATOR_VERSION) errors.push('validator_version_invalid');
  try {
    stablePayload(audit);
  } catch (error) {
...
function buildRuntimeQueueMaterializationAudit(input = {}) {
  const decision = isPlainObject(input.decision) ? input.decision : {};
  const result = isPlainObject(input.result) ? input.result : {};

  const audit = {
    audit_id: `runtime_queue_materialization_audit_${result.runtime_queue_materialization_result_id || decision.runtime_queue_materialization_decision_id || NOT_AVAILABLE_LABEL}`,
    runtime_queue_materialization_request_id: decision.runtime_queue_materialization_request_id || NOT_AVAILABLE_LABEL,
    runtime_queue_materialization_decision_id: decision.runtime_queue_materialization_decision_id || NOT_AVAILABLE_LABEL,
    runtime_queue_materialization_result_id: result.runtime_queue_materialization_result_id || NOT_AVAILABLE_LABEL,
    ...
    fingerprints: {
      runtime_queue_materialization_request_fingerprint: decision.runtime_queue_materialization_request_fingerprint || NOT_AVAILABLE_FINGERPRINT,
      runtime_queue_materialization_decision_fingerprint: result.runtime_queue_materialization_decision_fingerprint || NOT_AVAILABLE_FINGERPRINT,
    ...
    package_digest: decision.runtime_queue_materialization_package_digest || NOT_AVAILABLE_FINGERPRINT,
    ...
    status: decision.status || 'QUEUE_MATERIALIZATION_VALIDATION_FAILED',
    ...
    executed: false,
    validator_version: RUNTIME_QUEUE_MATERIALIZATION_AUDIT_VALIDATOR_VERSION
  };
...
module.exports = {
  ...
  NOT_AVAILABLE_LABEL,
  RUNTIME_QUEUE_MATERIALIZATION_AUDIT_FIELDS,
  RUNTIME_QUEUE_MATERIALIZATION_AUDIT_VALIDATOR_VERSION,
  buildRuntimeQueueMaterializationAudit,
  validateRuntimeQueueMaterializationAudit
};
