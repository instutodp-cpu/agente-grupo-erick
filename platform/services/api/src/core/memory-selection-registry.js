...
const { validateMemorySelectionItemReference } = require('./memory-selection-item-reference');
...
const { validateSelectionScore } = require('./memory-selection-score');
const { validateSelectionPlan } = require('./memory-selection-plan');
const { validateSelectionDecision } = require('./memory-selection-decision');

const MEMORY_SELECTION_REGISTRY_VALIDATOR_VERSION = 'memory_selection_registry_validator_v1';
const MEMORY_SELECTION_REGISTRY_STATUSES = Object.freeze([
...
const MEMORY_SELECTION_REGISTRY_SAFE_FLAGS = Object.freeze({
  simulation: true,
...
function safe(payload) {
  return cloneFrozen({ ...payload, ...MEMORY_SELECTION_REGISTRY_SAFE_FLAGS });
}
...
function createMemorySelectionRegistry() {
  const requestStore = createEntityStore({
    idField: 'selection_request_id', versionField: 'selection_request_version', tenantField: 'tenant_id',
    organizationField: 'organization_id', validate: validateMemorySelectionRequest, idLabel: 'selection_request'
  });
  const itemReferenceStore = createEntityStore({
    idField: 'item_reference_id', versionField: 'item_version', tenantField: 'tenant_id',
    organizationField: 'organization_id', validate: validateMemorySelectionItemReference, idLabel: 'item_reference'
  });
  const policyStore = createEntityStore({
    idField: 'policy_id', versionField: 'policy_version', validate: validateSelectionPolicy, idLabel: 'selection_policy'
  });
  const budgetStore = createEntityStore({
    idField: 'budget_id', validate: validateSelectionBudget, idLabel: 'selection_budget'
  });
  const scoreStore = createEntityStore({
    idField: 'item_reference_id', validate: validateSelectionScore, idLabel: 'selection_score'
  });
  const planStore = createEntityStore({
    idField: 'plan_id', tenantField: 'tenant_id', organizationField: 'organization_id',
    validate: validateSelectionPlan, idLabel: 'selection_plan'
  });
  const decisionStore = createEntityStore({
    idField: 'decision_id', tenantField: 'tenant_id', organizationField: 'organization_id',
    validate: validateSelectionDecision, idLabel: 'selection_decision'
  });
  ...
  return Object.freeze({
    ...
    registerItemReference: itemReferenceStore.register,
    getItemReferenceById: itemReferenceStore.getById,
    listItemReferencesByTenant: itemReferenceStore.listByTenant,
    listItemReferencesByOrganization: itemReferenceStore.listByOrganization,
    ...
    registerScore: scoreStore.register,
    getScoreById: scoreStore.getById,
...
module.exports = {
  MAX_LIST_RESULTS,
  MEMORY_SELECTION_REGISTRY_SAFE_FLAGS,
  MEMORY_SELECTION_REGISTRY_STATUSES,
  MEMORY_SELECTION_REGISTRY_VALIDATOR_VERSION,
  createMemorySelectionRegistry
};
