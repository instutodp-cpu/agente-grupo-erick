...
function createHermesMaintainerGithubReadRuntime({fetchImpl,resolveAuthorization,timeoutMs=DEFAULT_TIMEOUT_MS}={}){
 ...
 if(!Number.isInteger(timeoutMs)||timeoutMs<1||timeoutMs>30000)throw new TypeError('timeoutMs invalid');
 return Object.freeze({async readGithubRepositoryPath(input){
  ...
  let response;
  try{response=await fetchImpl(url,{method:'GET',redirect:'error',headers:{Accept:'application/vnd.github.raw+json',Authorization:authorization,'X-GitHub-Api-Version':'2022-11-28'},timeout_ms:timeoutMs});}
