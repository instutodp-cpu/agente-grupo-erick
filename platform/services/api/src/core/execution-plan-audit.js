...
const EXECUTION_PLAN_AUDIT_FIELDS = Object.freeze([
  'audit_id', 'execution_plan_request_id', 'execution_plan_id', 'fingerprints', 'tenant_binding',
  'organization_binding', 'project_binding', 'session_binding', 'agent_binding', 'task_binding', 'counts',
  'estimated_budget', 'side_effect_classifications', 'stop_condition_types', 'compensation_types', 'status',
  'decision', 'next_state', 'blockers', 'reason_codes', 'logical_sequence', 'simulation', 'production_blocked',
  'executed', 'validator_version'
]);

const FINGERPRINT_KEYS = Object.freeze([
  'request', 'authz', 'evidence_bundle', 'planning_result', 'orchestration_plan', 'task', 'execution_plan'
]);
...
function validateExecutionPlanAudit(audit) {
  ...
  for (const field of ['audit_id', 'execution_plan_request_id', 'execution_plan_id', 'status', 'decision', 'next_state', 'validator_version']) {
