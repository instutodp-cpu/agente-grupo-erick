...
| `project_state_reference`, `continuity_summary_reference`, `memory_selection_policy_reference` | `model-contract.js`'s generic `validateSingleReference` (PR #83) — same minimal reference shape as `workflow_reference`/`task_reference`/`budget_reference` above |
| `user_preference_references`, `required_memory_references` | `model-contract.js`'s generic `validateReferenceList` (PR #83) — same shape as `tool_references` above |

No field on `OrchestratorRequest` carries content, a message, or a prompt — confirmed both by the field list itself and by a dedicated test asserting none of the 25 field names resembles one.

**Continuity references (added after the initial PR #92 draft).** `user_preference_references`, `project_state_reference`, `continuity_summary_reference`, `required_memory_references`, and `memory_selection_policy_reference` are five additional reference-only fields declaring *that* a request carries user preferences, project state, a continuity summary, required memory items, and a memory selection policy — never the content of any of them, and never which items are actually selected. All five reuse the exact same minimal reference shapes already used elsewhere in this contract (`validateSingleReference`/`validateReferenceList`), are required (not optional), and participate in the same exact-fields/fingerprint/tenant/organization/deep-freeze/defensive-clone machinery as every other field on this contract — no new validation mechanism was introduced for them.
...
- **The memory selection policy is deliberately not implemented here.** `user_preference_references`/`required_memory_references`/`memory_selection_policy_reference` declare *that* these inputs exist, but this PR does not rank memory, does not classify anything as REQUIRED/RELEVANT/OPTIONAL, does not budget memory, does not deduplicate, does not detect superseded or conflicting items, does not handle overflow, and does not retrieve any memory content — no `orchestrator-memory-selection.js` exists, and `orchestrator-plan.js` was not changed to decide which memories are included. The full policy is scoped to **PR #93 — Hermes Memory Retrieval and Continuity Policy**.
...
**PR #93 — Hermes Memory Retrieval and Continuity Policy** will implement the actual memory selection policy (REQUIRED/RELEVANT/OPTIONAL classification, memory budget, deduplication, superseded/conflicted detection, overflow handling) against the reference fields added here. A separate future PR must add the evaluator (`orchestrator-engine.js`) that actually composes an `OrchestratorRequest` into an `OrchestratorPlan`/`OrchestratorDecision`, closing the cross-reference validation gap and the Tool/Workflow capability and side-effect cross-checks (`INTEG-04`, `INTEG-05`) identified in the PR #90 audit and its companion readiness document.
