...
The engine has no numeric budget ceiling of its own — `orchestrator-decision-request.js` deliberately carries no `plan_budget` field (PR #94's `PlanBudget` isn't part of this contract). Instead, the engine trusts `planning_result_reference.budget_validated`, which PR #94 already computed: `budget_validated=false` with `plan_generated=true` produces the resolvable `WAITING_BUDGET_REFERENCE` / `REQUEST_BUDGET_REVIEW`; `budget_validated=false` with `plan_generated=false` (no plan exists at all to review) produces the hard `BUDGET_BLOCKED`. **Não aumentar orçamento automaticamente** — nothing here changes any budget value.

## Dependências

`orchestration_plan_reference` only carries dependency *ids*, not full dependency objects (matching every other minimal reference in this PR's philosophy) — so real cycle detection needs the full graph. The engine accepts an optional `context.dependencyRecords` side-channel (full `PlanDependency`-shaped objects, reusing PR #94's own `hasDependencyCycle`) for tests and callers that have that graph; a detected cycle is `DEPENDENCY_BLOCKED`. A caller-supplied `context.pendingDependencyReviewIds` intersecting the plan's own dependency ids produces the resolvable `WAITING_DEPENDENCY_REFERENCE` / `REQUEST_DEPENDENCY_REVIEW`. An inconsistency between `planning_result_reference.dependency_ids` and `orchestration_plan_reference.dependency_ids` — the same plan described two different ways — is also `DEPENDENCY_BLOCKED`.

## Conflitos

Like dependencies, conflict detection has no data of its own on either minimal reference, so it is driven by an optional `context.unresolvedConflictIds` (hard `CONFLICT_BLOCKED`) / `context.resolvableConflictIds` (`WAITING_CONFLICT_RESOLUTION` / `REQUEST_CONFLICT_RESOLUTION`) side-channel.
...
- **Budget and dependency/conflict checks depend on data this contract deliberately doesn't carry.** Real numeric budget ceilings live only in PR #94's `PlanBudget`, not in this PR's `DecisionRequest`; real dependency/conflict graphs live only as ids on the minimal `OrchestrationPlanReference`. Both are handled via trusted upstream flags (`budget_validated`) or optional side-channel context parameters (`dependencyRecords`, `unresolvedConflictIds`, `resolvableConflictIds`) rather than inventing new contract fields not in the specification.
