...
### Network e Secret Policy Match (pr106fix / pr106fix2)
...
`runtime-worker-assignment-boundary.js`'s `evaluatePolicyReferenceMatch` localiza a policy oficial pelo ID declarado no binding e exige, nesta ordem: (1) o binding aponta para o próprio worker (`network_policy_reference_id === worker.network_policy_reference_id`) e carrega tenant/organização/projeto/ambiente corretos — as mesmas checagens de binding minimizado do pr106fix original; (2) a policy oficial referenciada existe (`worker_network_official_policy_missing` se não); (3) `official_network_policy_version === officialPolicy.destination_ref_version` (`worker_network_official_policy_version_mismatch` se não); (4) o fingerprint recomputado da policy oficial — usando o canonicalizador do próprio módulo oficial (`transcription-provider-contract-registry.js`'s `stablePayload`, nunca uma reimplementação paralela) — bate com `official_network_policy_fingerprint` (`worker_network_official_policy_fingerprint_mismatch` se não; isso captura tanto adulteração do conteúdo oficial quanto um binding com fingerprint desatualizado); (5) escopo — tenant para secret (`TranscriptionSecretReference` carrega `tenant_id` nativamente); (6) `officialPolicy.environment !== 'PRODUCTION'` (`worker_*_official_policy_not_allowed`). `network_policy_reference_valid`/`secret_policy_reference_valid` continuam existindo como flags derivadas de conveniência — nunca a fonte de verdade.

O requisito de rede/secret de cada estágio é derivado das próprias referências oficiais upstream já disponíveis (`model_selection_reference_id`, `tool_reference_ids`, `workflow_reference_id`) — um estágio puramente determinístico (sem modelo, tool ou workflow) genuinamente não tem requisito, representado explicitamente como `NOT_APPLICABLE` (match verdadeiro via essa regra nomeada, nunca inferido de presença de string). Uma policy oficial com ID duplicado dentro de `network_permission_policy_references`/`secret_resolution_policy_references` é uma incoerência global (`WORKER_ASSIGNMENT_POLICY_BLOCKED`, `network_official_policy_registry_duplicate`/`secret_official_policy_registry_duplicate`), distinta de um mismatch por-worker. Nenhum segredo é resolvido; nenhuma rede é usada; `evaluatePolicyReferenceMatch` nunca contata um provedor real.
...
`runtime-worker-assignment-request.js` (29 campos exatos) agrega toda a cadeia do Scheduler já preparado (`runtime_scheduler_request_reference`/`_decision_reference`/`_result_reference`/`_package_reference`), o Runtime Execution Package e Stage Manifest, Capacity Snapshot/Concurrency/Freshness — **reutilizados verbatim**, nunca contratos paralelos — mais os 8 catálogos declarativos: `runtime_worker_references`/`_capability_references`/`_capacity_references`/`_health_references` (pr106), `runtime_worker_network_policy_references`/`_secret_policy_references` (bindings minimizados, pr106fix), e `network_permission_policy_references`/`secret_resolution_policy_references` (as próprias policies oficiais reutilizadas verbatim, pr106fix2) — até 200 itens cada. `runtime_replay_reference`/`idempotency_reference` são a mesma `RuntimeReadinessReplayReference`/`ExecutionPlanIdempotencyReference` que fluiu desde a Admission — a continuidade é provada exigindo byte-identidade (mesmo fingerprint) com o par carregado pelo próprio `runtime_scheduler_request_reference`, nunca uma re-derivação independente. `context` nunca é lido para nenhuma decisão.
...
- (pr106fix2) Os contratos oficiais reutilizados são do domínio de transcrição (carregam `provider_slug`, `transport_id`, etc.) e não possuem um conceito nativo de "runtime worker environment" ou de allowlist por tool/workflow — o binding prova genuinamente que a policy oficial existe, tem o conteúdo declarado (ID/versão/fingerprint) e (para secret) pertence ao tenant correto, mas não impõe um allowlist granular de tool/workflow que os contratos oficiais simplesmente não modelam. Documentado honestamente como limitação de domínio, não uma checagem inventada.
