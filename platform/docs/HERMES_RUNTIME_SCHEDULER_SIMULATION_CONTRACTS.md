...
**Estágios com dependências required permanecem WAITING_DEPENDENCY_REFERENCE porque nenhuma dependência é satisfeita nesta PR.** A derivação de elegibilidade (`runtime-scheduler-boundary.js`'s `deriveStageStatus`) segue, nesta ordem: (1) violação estrutural/policy (efeito externo/irreversível, tipo de stage não permitido pela policy) → `BLOCKED`; (2) qualquer dependência de entrada com `required=true` (nenhuma jamais satisfeita nesta PR) → `WAITING_DEPENDENCY_REFERENCE`; (3) `approval_required=true` → `WAITING_APPROVAL_REFERENCE`; (4) `optional=true` → `OPTIONAL_REFERENCE`; (5) caso contrário → `ELIGIBLE_REFERENCE_SIMULATION`. "Isso ainda não significa que será executado" — nenhum flag operacional (`job_created`, `queue_used`, `worker_assigned`, `stage_dispatched`, `stage_started`, `stage_completed`, `stage_failed`, `scheduler_stage_applied`) é jamais forçado `true`, independentemente do status.

**(pr105fix) Somente dependências required bloqueiam a elegibilidade. Dependências opcionais são preservadas como referências, mas não impedem o estágio de se tornar eligible, waiting approval ou optional.** O mapa de "dependência de entrada" que alimenta `deriveStageStatus` é construído filtrando `dependency.required === true` antes de qualquer outra coisa — uma dependência `required=false` continua presente em `dependency_reference_ids` (preservação 1:1 da referência), mas nunca em `blocking_dependency_reference_ids` e nunca altera `scheduler_stage_status`/`eligibility_status` por si mesma. Um estágio com uma dependência apenas opcional, sem `approval_required` e sem ser `optional`, alcança `ELIGIBLE_REFERENCE_SIMULATION` normalmente.
...
**(pr105fix) A ordem do Scheduler Package é uma ordenação topológica estável das dependências required. Nenhum target required aparece antes do predecessor.** `runtime-scheduler-boundary.js`'s `topologicalOrderStages` implementa um Kahn determinístico restrito a arestas `required=true` (arestas opcionais nunca impõem restrição de ordem): (1) indegree e adjacência construídos somente sobre as arestas required; (2) todo estágio com indegree zero entra no conjunto disponível; (3) o próximo estágio escolhido do conjunto disponível usa o desempate de 6 níveis: estruturalmente bloqueado por último, `approval_required` depois de sem-aprovação, `optional` depois de required, maior `priority` primeiro, menor `stage_sequence` primeiro, menor `runtime_stage_reference_id` lexicograficamente; (4) o estágio escolhido é removido e o indegree dos seus alvos é reduzido; (5) o processo repete até esvaziar o conjunto disponível. Se nem todo estágio for ordenado (ciclo entre arestas required), o resultado é `SCHEDULER_DEPENDENCY_BLOCKED`/`scheduler_required_dependency_cycle_detected` — o scheduler nunca confia apenas na flag `cycle_free` já validada upstream (passo 11), falhando de forma segura mesmo que essa flag esteja incorretamente `true`. Após ordenar, cada dependência required é explicitamente revalidada (`position(from) < position(to)`); uma violação bloqueia como `SCHEDULER_DEPENDENCY_BLOCKED`/`scheduler_required_predecessor_order_violation::<dependency_id>`. Nenhuma dependência é satisfeita, nenhum target se torna eligible por causa da ordem, nenhum estágio é iniciado. Nenhum `context` altera a ordem. **Nenhum job, queue, worker ou dispatch é criado.**

Como nenhum predecessor é jamais concluído nesta PR, todo estágio-alvo de uma dependência required permanece `WAITING_DEPENDENCY_REFERENCE` — mas sua posição na ordem final continua sendo uma garantia estrutural do algoritmo (nunca antes do predecessor), não apenas um efeito colateral da classificação. Dependências opcionais nunca restringem essa ordem.
