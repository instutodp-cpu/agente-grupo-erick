...
`CONNECTOR_LIFECYCLE_RUNTIME_REGISTRY.md` adds deterministic connector lifecycle
scenarios for registration, candidate nomination, mock-only enablement,
readiness pending/passed/blocked, version conflict and blocked future
activation. These scenarios do not change `/message` or `/confirm`.
